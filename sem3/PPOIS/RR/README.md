...
### 1. StorageOptimizationAgent
**Назначение:** Оркестрация всего процесса оптимизации размещения складов.

**Входные данные:**
- Адрес сущности, содержащей путь к CSV-файлу

**Выходные данные:**
- Финальный результат с рекомендацией

**Особенности:**
- Координирует работу всех агентов
- Обрабатывает ошибки на каждом этапе
- Обеспечивает последовательное выполнение пайплайна
- Возвращает итоговую рекомендацию по оптимальному размещению складов

---

### 2. FileLoaderAgent
**Назначение:** Загрузка и парсинг CSV-файла с данными о магазинах.

**Входные данные:**
- Адрес сущности, содержащей путь к CSV-файлу
  
**Выходные данные:**
- Структура торговой сети (concept_network) с магазинами

**Особенности:**
- Поддерживает CSV с разделителем ```;```
- Обрабатывает ошибки чтения файла
- Очищает предыдущие результаты загрузки

---

### 3. DataVerifierAgent
**Назначение:** Проверка корректности входных данных торговой сети.

**Входные данные:**
- Адрес узла торговой сети (```concept_network```)

**Выходные данные:**
- Результат валидации с количеством магазинов и флагом валидности

**Особенности:**
- Проверяет наличие минимум 3 магазинов
- Верифицирует обязательные атрибуты каждого магазина
- Устанавливает флаг ```nrel_is_valid``` для дальнейшего использования

---

### 4. ShopClusteringAgent
**Назначение:** Кластеризация магазинов методом K-means для определения оптимальных позиций складов.

**Входные данные:**
- Адрес торговой сети (```concept_network```)
- Значение K (количество кластеров)

**Выходные данные:**
- Вариант размещения (```concept_placement_variant```) с координатами складов

**Особенности:**
- Использует взвешенный центроид для определения координат складов
- Ограничивает K от 1 до 10
- Сохраняет список магазинов для каждого склада

---

### 5. CostEstimatorAgent
**Назначение:** Расчет транспортных затрат для варианта размещения складов.

**Входные данные:**
- Адрес варианта размещения (`concept_placement_variant`)
- Адрес торговой сети (`concept_network`)

**Выходные данные:**
- Транспортные затраты, связанные с вариантом через `nrel_transport_cost`

**Особенности:**
- Использует евклидово расстояние
- Тариф фиксирован: 50 руб/(км·т)
- Магазин приписывается к ближайшему складу

---

### 6. VariantSelectorAgent
**Назначение:** Сравнение вариантов размещения и выбор оптимального.

**Входные данные:**
- Три адреса вариантов размещения (для K=1, K=2, K=3)

**Выходные данные:**
- Рекомендация с указанием лучшего варианта

**Особенности:**
- Рассчитывает процент экономии относительно варианта с 1 складом
- При равенстве затрат выбирает вариант с меньшим K
- Добавляет атрибут экономии для каждого варианта
